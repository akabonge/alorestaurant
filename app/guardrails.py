...
SESSION_RATE_LIMIT = 25        # max messages per session per window
SESSION_RATE_WINDOW = 300      # 5-minute rolling window

# ── Rate limiting — IP-based ──────────────────────────────────────────────────
_ip_rate_store: dict[str, list[float]] = {}
_ip_rate_lock = threading.Lock()
IP_RATE_LIMIT = 120            # max requests per IP per window
IP_RATE_WINDOW = 3600          # 1-hour rolling window
...
def check_input(text: str, session_id: str, ip: str = "") -> GuardrailResult:
    ...
    for pattern in _COMPILED:
        if pattern.search(text):
            return GuardrailResult(
                allowed=False,
                reason="I'm only here to help with questions about our restaurant. What can I help you with?",
            )
